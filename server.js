...
app.post('/webhook/lead', (req, res) => {
  ...
  try {
    ...
    // Not wired to the GHL push (unused in production; many callers here are
    // GHL itself, so pushing back would be circular). Also never trust a
    // caller-supplied ghlPushed value.
    data.ghlPushed = 'excluded_source';
...
app.post('/webhook/google-ads-lead', (req, res) => {
  ...
  // AFTER responding — Google retries a slow response, and the GHL push makes
  // several outbound calls. pushAfterUpsert never throws.
  if (lead && result) pushAfterUpsert(lead, result);
});
...
// `require()`'d, so a test can import the app without opening a real port,
// starting the 15-minute sync loop, or making any of the outbound calls
// runFullSync() would otherwise kick off 5 seconds after every test run. `require.main === module` is Node's standard way to tell
...
if (require.main === module) {
  ...
  console.log(`GHL lead push mode: ${describeMode()}`);
